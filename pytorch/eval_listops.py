...
from experiment_utils.generate_data import chunk_data_loader
...
###########
args.eval_tgt_len = args.answer_size = args.tgt_len
#########
...
# Load dataset
va_iter = chunk_data_loader('val', path=args.data, task_name=args.dataset, batch_size=args.batch_size,
                                    tgt_len=args.tgt_len, device=device)
te_iter = chunk_data_loader('test', path=args.data, task_name=args.dataset, batch_size=args.batch_size,
                                    tgt_len=args.tgt_len, device=device)
ntokens = args.ntokens = (te_iter.src[0].max() + 1).item()
...
def evaluate(eval_iter):
    ...
    with torch.no_grad():
        ...
        for i, (data_, target_, seq_len) in enumerate(eval_iter):
            ...
            if data_.shape[0] > args.tgt_len:
                print('Data longer than context')
...
#                 print('data, target', data.shape, target.shape)
        ...
        # with teacher forcing
            ...
            if mems is None:
                mems = tuple()
            if not mems: mems = model.init_mems(data_.device)

            tgt_len = target_.size(0)
            hidden, mems = model._forward(data_, mems=mems, mem_tokens=mem_tokens)
            ...
            num_correct_all += (preds == target_).float().sum().item()
            num_total_all += target_.shape[0] * target_.shape[1]
            if 1 in target_:
    ...
    model.reset_length(args.tgt_len, args.ext_len, args.mem_len)
    model.train()
    print('num_correct, num_total', num_correct_tf, num_total)
    print('num_correct symbols, num_total symbols', num_correct_all, num_total_all)
    print('symbol accuracy ', num_correct_all / num_total_all)
